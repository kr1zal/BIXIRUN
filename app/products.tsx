...
if (ENABLE_OPTIMIZATIONS.enableMetrics) {
    console.log('[Products] Feature flags:', ENABLE_OPTIMIZATIONS);
}
...
// ✅ ОПТИМИЗИРОВАННЫЙ КЕШ СЕЛЕКТОРОВ с LRU и WeakMap
class SelectorCache {
    private cache: Map<string, ReturnType<typeof makeSelectCartItem>> = new Map();
    private usage: Map<string, number> = new Map();
    private maxSize = 100;

    get(productId: string): ReturnType<typeof makeSelectCartItem> {
        if (!this.cache.has(productId)) {
            // Если кеш переполнен, удаляем наименее используемый
            if (this.cache.size >= this.maxSize) {
                const leastUsed = Array.from(this.usage.entries())
                    .sort((a, b) => a[1] - b[1])[0];
                if (leastUsed) {
            ...
            this.cache.set(productId, makeSelectCartItem());
        }
        
        // Обновляем счетчик использования
        this.usage.set(productId, Date.now());
...
// ✅ Используем WeakMap для автоматической очистки при unmount компонентов
const cartSelectorsCache = ENABLE_OPTIMIZATIONS.useWeakMapCache 
    ? new SelectorCache() 
    : new Map<string, ReturnType<typeof makeSelectCartItem>>();

const getCartItemSelector = (productId: string) => {
    if (ENABLE_OPTIMIZATIONS.useWeakMapCache) {
        return (cartSelectorsCache as SelectorCache).get(productId);
    } else {
        // Старая логика для обратной совместимости
        if (!cartSelectorsCache.has(productId)) {
            cartSelectorsCache.set(productId, makeSelectCartItem());
        }
        return cartSelectorsCache.get(productId)!;
    }
};

// Удален неиспользуемый компонент QuantityCounter

// ✅ КАСТОМНЫЙ ХУК для изоляции обновлений корзины
const useCartItem = (productId: string) => {
    const selector = useMemo(
        () => getCartItemSelector(productId),
        [productId]
    );
    
    const cartItem = useAppSelector(state => selector(state, productId));
    ...
    }), [productId, cartItem?.quantity, dispatch]);
...
const GridItem = React.memo(({ item, onPress }: CardProps) => {
    // ✅ ОПТИМИЗИРОВАНО: Используем новый хук для изоляции обновлений
    const { quantity, increment, decrement, addToCart } = useCartItem(item.id);
...
export default function ProductsScreen() {
    ...
    // ✅ ОПТИМИЗИРОВАННЫЙ getItemLayout с учетом реальных отступов
    const getOptimizedItemLayout = useCallback((_, index) => {
        if (viewMode === 'grid') {
            const row = Math.floor(index / 2);
            const itemHeightWithGap = GRID_CARD_HEIGHT + GRID_GAP;
            return {
                length: GRID_CARD_HEIGHT,
                offset: row * itemHeightWithGap,
                index
            };
        } else {
            const LIST_ITEM_GAP = 6; // marginBottom из стилей
            const itemHeightWithGap = LIST_CARD_HEIGHT + LIST_ITEM_GAP;
            return {
                length: LIST_CARD_HEIGHT,
                offset: index * itemHeightWithGap,
                index
            };
        }
    }, [viewMode]);
